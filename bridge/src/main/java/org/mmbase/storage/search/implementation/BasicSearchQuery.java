...
public class BasicSearchQuery implements SearchQuery, org.mmbase.util.PublicCloneable<BasicSearchQuery>,  SizeMeasurable, java.io.Serializable {
    ...
    protected void copyFields(SearchQuery q) {
        ...
        for (StepField field : q.getFields()) {
            ...
            BasicStepField newField = addField(newStep, ((BasicStepField) field).getField());
    ...
    protected void copySortOrders(SearchQuery q) {
        ...
        for (SortOrder sortOrder : q.getSortOrders()) {
            ...
            if (j == -1 || j >= fields.size()) { // not sorting on field of field list.
                Step step = field.getStep();
                newField = new BasicStepField(field.getStep(), ((BasicStepField) field).getField());
            } else {
    ...
    protected static StepField createNewStepField(SearchQuery q, StepField f) {
        ...
        return new BasicStepField(step, ((BasicStepField) f).getField());
    }
