	...
	*/
	...
	public String doSearchUserNumber(String value) {
		...
		try {
			...
			ResultSet rs   = stmt.executeQuery ("select parent from vpro4_properties where (key='LID' AND value='" + value
				+ "') union select parent from vpro4_properties where (key='EMAIL' AND value='" + value + "') ");
			...
			debug ("SQLException while searching for " + value);
			e.printStackTrace ();
		}
