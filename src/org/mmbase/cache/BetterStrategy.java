...
public class BetterStrategy extends AbstractReleaseStrategy {
    ...
    public boolean doEvaluate(NodeEvent event, SearchQuery query, List cachedResult) {
        ...
        //first lets test if the query has one step and the event is a relation event
        ...
        //The role need not allways be defined in the query. if an undefined role exists 
        //between the source and the target of the relation event, this optimization
        //can not be done
        if(shouldRelease && query.getSteps().size() > 1 && event instanceof RelationEvent){
