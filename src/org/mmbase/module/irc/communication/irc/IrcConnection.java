...
public class IrcConnection extends TcpipConnection implements CommunicationInterface {
     ...
     * @javadoc
     *
     * @param com  Description of Parameter
     */
                ...
                if (checkint("parseMessage", "message.command()", message.command())) {
                    ...
                    if (checkint("parseMessage", "message.command()", message.command())) {
                        ...
                        switch (number) {
                            ...
                            case 376:
                            {
                                log.debug("parseMessage(): 376 Signal that logged in");
                                accepted();
...
// -------------------------------------------------------------------
     ...
     *
     * @param server       Description of Parameter
     * @param channelname  Description of Parameter
     * @param channelkey   Description of Parameter
     */
...
// -------------------------------------------------------------------
    ...
    private boolean checkint(String method, String name, String s) {
        ...
        if (s == null) {
            log.error("string(" + name + ") is null!");
        } else
            if (s.equals("")) {
            log.error("string(" + name + ") is empty!");
        } else {
            try {
                Integer.parseInt(s);
                result = true;
            } catch (NumberFormatException e) {
                // log.error("string("+name+") is not a number!");
            }
        }
