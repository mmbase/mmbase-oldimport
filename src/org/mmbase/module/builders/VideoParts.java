...
public class VideoParts extends MMObjectBuilder {
	...
	public boolean create() {
		...
		try {
			...
			stmt.executeUpdate("create row type "+mmb.baseName+"_"+tableName+"_t (title varchar(255) not null"
				...
				+", intro char(2048)"
				+", body text"
				+", storage integer) under "+mmb.baseName+"_object_t");
			stmt.close();
			con.close();
	...
	*/
	public int insert(String owner,MMObjectNode node) {
		...
		int source=node.getIntValue("source");
		...
		try {
			...
			PreparedStatement stmt=con.prepareStatement("insert into "+mmb.baseName+"_"+tableName+" values(?,?,?,?,?,?,?,?,?,?)");
				...
				stmt.setInt(6,source);
				stmt.setInt(7,playtime);
				stmt.setString(8,intro);
				setDBText(9,stmt,body);
				stmt.setInt(10,storage);
				stmt.executeUpdate();
				stmt.close();
				con.close();
		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println("VideoParts -> Error on : "+number+" "+owner+" fake");
			return(-1);
		}
