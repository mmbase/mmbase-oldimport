 ...
 */
public class pccarddrvsProbe implements Runnable {

	Thread kicker = null;
	pccarddrvs parent=null;
	int trackNr=-1;
	String filename=null;
	MMObjectNode node=null;

	public pccarddrvsProbe(pccarddrvs parent,MMObjectNode node) {
		this.parent=parent;
		this.node=node;
		init();
	}
	...
	public void start() {
		...
		if (kicker == null) {
			kicker = new Thread(this,"pccarddrvs");
			kicker.start();
		}
	}
	...
	public void run() {
		...
		if (state.equals("getdir")) {
			System.out.println("pccarddrvsProbe:run(): state='getdir' !!!");
			try {
				...
				System.out.println("pccarddrvsProbe:run():"+Exc+gdfe.errval+"  "+gdfe.explanation);
			...
			System.out.println("pccarddrvsProbe:run(): state='copy' !!!!");
                        try {
				try {   
					...
					System.out.println("pccarddrvsProbe:run():"+Exc+cfe.errval+"  "+cfe.explanation);
					node.setValue("state","error");
