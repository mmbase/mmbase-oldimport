...
public class MMHypersonic2Node extends MMSQL92Node implements MMJdbc2NodeInterface {
	...
	public MMObjectNode decodeDBnodeField(MMObjectNode node,String fieldtype,String fieldname, ResultSet rs,int i,String prefix) {
			try {
				if (fieldtype.equals("VARSTRING") || fieldtype.equals("CHAR") || fieldtype.equals("VARCHAR")) {
					String tmp=rs.getString(i);
					System.out.println("VAL="+fieldname+" "+tmp);
					if (tmp==null) {
					...
					System.out.println("MMHypersonicNode mmObject->"+fieldname+"="+fieldtype+" node="+node.getIntValue("number"));
				}
			} catch(SQLException e) {
				System.out.println("MMhypersonic2Node mmObject->"+fieldname+"="+fieldtype+" node="+node.getIntValue("number"));
				e.printStackTrace();	
			}
			return(node);
	}
	...
	*/
	public String getDBText(ResultSet rs,int idx) {
		...
		try {
			...
			if (inp==null) {
				//System.out.println("MMObjectBuilder -> MMhypersonic2Node DBtext no ascii "+inp);
				 return("");
			}
			if (rs.wasNull()) {
				System.out.println("MMObjectBuilder -> MMHypersonic2Node DBtext wasNull "+inp);
				return("");
			}
			siz=inp.available(); // DIRTY
