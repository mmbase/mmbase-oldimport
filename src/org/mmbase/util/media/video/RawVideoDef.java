...
//  -------------------------	
	...
	public RawVideoDef( MMObjectNode node )
	{
		...
		//  -------------------------------------------	
			...
			debug("RawVideoDef("+node+"): ERROR: node is null!"); 
	}
	...
	public String getRealVideoUrl( scanpage sp )
	{
		...
		if( status == STATUS_GEDAAN )
		{
			...
			if( format == FORMAT_G2 )
			{
				...
				debug("getRealVideoUrl(): ERROR: For number("+number+"): Unknown format("+format+")");
		}
		else 
			debug("getRealVideoUrl(): ERROR: For number("+number+"): Asking url while status("+status+") signals 'not finished yet'!");
