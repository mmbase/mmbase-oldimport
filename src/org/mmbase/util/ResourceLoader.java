...
public class ResourceLoader extends ClassLoader {
	...
	protected ResourceLoader(final ResourceLoader cl, final String context) {
		...
		while (i.hasNext()) {
			...
			if (o instanceof FileURLStreamHandler) {
				roots.add(((FileURLStreamHandler) o).getChildStreamHandler(context));
	...
	public Document getDocument(String name) throws org.xml.sax.SAXException, IOException {
		...
		DocumentBuilder dbuilder = org.mmbase.util.xml.DocumentReader.getDocumentBuilder(true, null /* no error handler */
		, resolver);
		if (dbuilder == null)
	...
	protected class FileURLStreamHandler extends PathURLStreamHandler {
		...
		FileURLStreamHandler(FileURLStreamHandler f) {
			fileRoot = f.fileRoot;
			log.info("new FileUrlStreamHandler with file root " + f.fileRoot);
				...
				try {
					...
					BasicFieldValueConstraint nameConstraint =
						new BasicFieldValueConstraint(
							...
							ResourceLoader.this.context.getPath().substring(1) + "%");
