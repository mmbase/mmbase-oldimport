...
public class Detector {
      ...
      if (level == 1) {
         ...
         if (level > 1) {
            if (childList.size() == 0) {
               ...
               ((Detector) childList.get(childList.size() - 1)).addChild(
                  detector,
                  level - 1);
            }
         }
   ...
   protected boolean testString(byte[] lithmus) {
      ...
      if (lithmusString.length() >= offset + test.length()) {
         ...
         String compare =
            lithmusString.substring(offset, offset + test.length());
         xString = compare;
         int n;
   ...
   protected boolean testShort(byte[] lithmus, int endian) {
      ...
      if (endian == BIG_ENDIAN) {
         found =
            byteArrayToInt(new byte[] { lithmus[offset], lithmus[offset + 1] });
      }
      else
         if (endian == LITTLE_ENDIAN) {
            found =
               byteArrayToInt(
                  new byte[] { lithmus[offset + 1], lithmus[offset] });
         }
      xInt = found;
   ...
   protected boolean testLong(byte[] lithmus, int endian) {
      ...
      try {
         ...
         if (!message.equals("")) {
            log.error(
               "Failed to test " + label[endian] + " long for " + message);
         }
         else {
