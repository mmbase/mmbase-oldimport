...
public class MagicXMLReader
   ...
   private static Logger log =
      Logging.getLoggerInstance(MagicXMLReader.class.getName());
   ...
   private static void setReader(File file) throws IllegalArgumentException {
      if (!file.exists()) {
         throw new IllegalArgumentException(
            "magic file  " + file + " does not exist");
      }
   ...
   public synchronized static MagicXMLReader getInstance() {
      if (reader == null) { // can only occur once.
         ...
         try {
            ...
            log.info(
               "The file does not exist, cannot create MagicXMLReader instance");
            return null;
         }
      ...
      while (p < s.length()) {
         ...
         if (c == '\\') {
            if (p > s.length() - 4) {
               ...
               if (!failed) {
                  buf.append(s.substring(stoppedAt, p)).append(
                     (char) Integer.parseInt(s.substring(p + 1, p + 4), 8));
                  stoppedAt = p + 4;
                  p = p + 4;
               }
               else {
