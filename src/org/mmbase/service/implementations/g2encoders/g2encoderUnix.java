...
import org.mmbase.remote.StringTagger;
 ...
 * @author Marcel
 * @version $Id: g2encoderUnix.java,v 1.9 2002-04-29 10:54:18 pierre Exp $
 */
public class g2encoderUnix implements g2encoderInterface {
    ...
    public int doEncode(String cmds) {
        ...
        options+=" -t 2,3,4";
        log("doEncode(): exec("+ENCODER_PATH+options+")");
        int exitValue = execute(ENCODER_PATH+options);
        log("doEncode(): encoding done, exitvalue="+exitValue);
        return exitValue;
    }
    ...
    private int execute (String command) {
        ...
        try {
            ...
            log("execute: ERROR getting info from stdout, sofar I've read: "+s+", returning 1 as exitvalue.");
            e.printStackTrace();
            return 1;
        }
        ...
        try {
            ...
            log("execute: ERROR getting info from stderr, sofar I've read: "+s+", returning 1 as exitvalue.");
            e.printStackTrace();
            return 1;
        }
    ...
    /**
     * @javadoc
     * @param params 28k, 56k, singleISDN, dualISDN, lan, cable - implemented with ignoreCase
     */
        ...
        if (params != null) {
            if( !params.equals("")) {
                if(	params.equalsIgnoreCase("28k")) {
                    result = "0";
                } else if( params.equalsIgnoreCase("56k")) {
                    result = "1";
                } else if( params.equalsIgnoreCase("singleISDN")) {
                    result = "2";
                } else if( params.equalsIgnoreCase("dualISDN")) {
                    result = "3";
                } else if( params.equalsIgnoreCase("lan")) {
                    result = "4";
                } else if( params.equalsIgnoreCase("cable")) {
                    result = "5";
                } else {
                    // signal error and return lowest quality
                    // --------------------------------------
                    log("toNum("+params+"): ERROR: This is not a valid option! - Valid options are: 28k,56k,singleISDN,dualISDN,lan and cable.");
                    result = "0";
                }
            } else {
                // signal error and return lowest quality
                // --------------------------------------
                log("toNum("+params+"): ERROR: params is empty ! - Valid options are: 28k,56k,singleISDN,dualISDN,lan and cable.");
                result = "0";
            }
        } else {
            // signal error and return lowest quality
            // --------------------------------------
            log("toNum("+params+"): ERROR: params is null! - Valid options are: 28k,56k,singleISDN,dualISDN,lan and cable.");
            result = "0";
        }
        return result;
    }
        ...
        if( format != null ) {
            if( !format.equals("") ) {
                if( format.equalsIgnoreCase("voice") ) {
                    ...
                    log("audioFormat("+format+"): ERROR: parameter format is not legal! Valid options are: voice, voice with music, mono music, stereo music.");
                    result = "0";
                }
            } else {
                log("audioFormat("+format+"): ERROR: parameter format is empty! Valid options are: voice, voice with music, mono music, stereo music.");
                result = "0";
            }
        } else {
            log("audioFormat("+format+"): ERROR: parameter format is null! Valid options are: voice, voice with music, mono music, stereo music.");
            result = "0";
        }
        return result;
    }
        ...
        if ( format != null ) {
            if ( !format.equals("") ) {
                if( format.equalsIgnoreCase("normal") ) {
                    ...
                    log("videoFormat("+format+"): ERROR: parameter format is not legal! Valid options are: normal, smooth, sharp, slideshow.");
                    result = "0";
                }
            } else {
                log("videoFormat("+format+"): ERROR: parameter format is empty! Valid options are: normal, smooth, sharp, slideshow.");
                result = "0";
            }
        } else {
            log("videoFormat("+format+"): ERROR: parameter format is null! Valid options are: normal, smooth, sharp, slideshow.");
            result = "0";
        }
        return result;
    }
        ...
        if( device != null ) {
            if( !device.equals( "" ) ) {
                if( device.equalsIgnoreCase( "audioFile" )) {
                    ...
                    try {
                        ...
                        if( deviceNr > 0 && deviceNr < 5 ) {	// 1 .. 4 are valid numbers
                            ...
                            if( format.equalsIgnoreCase( "television" )) {
                                ...
                                log( "captureDevice("+device+"): ERROR: No valid format("+format+") specified for videoCapture("+(deviceNr-1)+")! (Valid options are televsion/composite1/s-video).");
                                result = "" + deviceNr + ":0";
                            }
                        } else {
                            log( "captureDevice("+device+"): ERROR: videoCapture has to be between 1..4 (but specified number("+deviceNr+"). Using standard 2:0 (/dev/video0 - television) as device.");
                            result = "2:0";
                        }
                    } catch( NumberFormatException nfe ) {
                        log( "captureDevice("+device+"): ERROR: While determining video deviceNumber: This is NOT a number("+sdeviceNr+")! Using standard 2:0 (/dev/video0 - television) as device.");
                        nfe.printStackTrace();
                        result = "2:0";
                    }
                    ...
                    try {
                        ...
                        if( deviceNr > 0 && deviceNr < 3 ) { // check between 1..2
                            ...
                            if( inputDevice.equalsIgnoreCase( "microphone" )) {
                                ...
                                log( "captureDevice("+device+"): ERROR: Found intputDevice("+inputDevice+") is not a valid inputdevice! Using standard device( audio0/microphone )." );
                                result = "" + deviceNr + ":0";
                            }
                        } else {
                            log( "captureDevice("+device+"): ERROR: Found inputdeviceNr("+deviceNr+") not in range(1..2)! Using standard device( audio0/microphone ) ");
                            result = "6:0";
                        }
                    } catch( NumberFormatException nfe ) {
                        log("captureDevice("+device+"): ERROR: While determining audio deviceNumber: This is NOT a number("+sdeviceNr+")! Using standard device( audio0/microphone ).");
                        nfe.printStackTrace();
                        result = "6:0";
                    }
                    ...
                    log("captureDevice("+device+"): ERROR: Found device("+device+") is not a valid device! Valid devices are audioFile, quicktimeFile, videoDevice[1..4], audioDevice[1..2] or windowCapture. Returning device 0:0.");
                    result = "0:0";
                }
            } else {
                log( "captureDevice("+device+"): ERROR: captureDevice is empty! Returning device 0:0.");
                result = "0:0";
            }
        } else {
            log( "captureDevice("+device+"): ERROR: captureDevice is null! Returning device 0:0. ");
            result = "0:0";
        }
        return result;
    }
    ...
    public boolean checkstring( String method, String varname, String tocheck ) {
        ...
        if( method == null 	  ) log("checkstring("+method+","+varname+","+tocheck+"): ERROR: method("+method+") is null!");
        else if( method.equals("") ) log("checkstring("+method+","+varname+","+tocheck+"): ERROR: method("+method+") is empty!");
        else if( varname == null	  ) log("checkstring("+method+","+varname+","+tocheck+"): ERROR: varname("+varname+") is null!");
        else if( varname.equals("")) log("checkstring("+method+","+varname+","+tocheck+"): ERROR: varname("+varname+") is null!");
        else if( tocheck == null   ) log( method+"(): "+varname+"("+tocheck+") is null!");
        else if( tocheck.equals("")) log( method+"(): "+varname+"("+tocheck+") is empty!");
        else result = true;
        return result;
    }
}
