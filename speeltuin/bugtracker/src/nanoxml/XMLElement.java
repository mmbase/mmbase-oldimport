...
implements Serializable {
    ...
    /**
     * optional comment of the XMLElement
     * @author keesj@framfab.nl
     */
    ...
    /**
     * Add a subobject at a certain position
     **/
    public void addChildAt(XMLElement child,int index){
        child.parent =this;
    ...
    /**
     * Changes the comment string.
     * comments can only be writen
    ...
    int    indent) {
        ...
        if (comment != null){
            ...
            while(st.hasMoreTokens()){
                //this.writeEncoded(out, st.nextToken());
    ...
    /**
     * clone the element by copying the String values
     * @param deep if deep equals true the node and al it's sub nodes wil be cloned
     */
    public XMLElement clone(boolean deep){
        XMLElement xmle = new XMLElement();
        xmle.setTagName(getTagName());
        Enumeration enum =attributes.propertyNames();
        while(enum.hasMoreElements()){
            String propName = (String)enum.nextElement();
            xmle.addProperty(propName,getProperty(propName));
        }
        xmle.contents = contents;
        xmle.comment =comment;
        if (deep){
            java.util.Iterator iter = getChildren().iterator();
            while(iter.hasNext()){
                xmle.addChild( ((XMLElement)iter.next()).clone(deep));
            }
        }
        return xmle;
    }
