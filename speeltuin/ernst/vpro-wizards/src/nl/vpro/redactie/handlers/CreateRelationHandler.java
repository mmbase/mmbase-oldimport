...
/**
 * creates a new relation from a given source node to a given destination node with a given role. source and destination can be given as
 * either a specific node number of as a reference to a previously mapped node. all the values in the fields map will be set on the new
 * node. date or file fields are not supported. Errors caused by setting these values result in a FieldError instance in the
 * resultContainer.Errors map. <br/> With sortfield you can set a sorting value on the relation (think: posrel: pos). When the role is
 * posrel this fields defaults to pos. for other relations (specializations of posrel) you can set the field to use. the field must be of
 * type number. With sortPosition you can determine if the new relation should be added to the begin of the sorted list or the end.
 *
 * @author ebunders
 *
 */
public class CreateRelationHandler extends Handler<CreateRelationAction> {
    ...
    public CreateRelationHandler(Transaction transactionalCloud, ResultContainer resultContainer, Map<String, Node> idMap) {
    ...
    void process() {
        if (isValid()) {
            ...
            String sortPosition = (action.getSortPosition().isEmpty() ? "" : action.getSortPosition().toArray()[0].toString());
    ...
    Integer getPosition(String sortField, String sortPosition, Node source, Node destination, String role) {
        ...
        try {
            ...
            Query q = Queries.createQuery(transactionalCloud, source.getNumber() + "", source.getNodeManager().getName() + "," + role + ","
                    + destination.getNodeManager().getName(), role + "." + sortField, null, role + "." + sortField, (sortPosition
