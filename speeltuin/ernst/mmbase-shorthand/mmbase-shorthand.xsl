...
<xsl:stylesheet 
  ...
  <xsl:template match="ms:application" mode="application">
      ...
      <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE application PUBLIC "-//MMBase//DTD application config 1.1//EN" "http://www.mmbase.org/dtd/application_1_1.dtd"></xsl:text>
      <!-- version -->
        ...
        <xsl:variable name="autodep">
            <xsl:choose>
                <xsl:when test="@autodeploy"><xsl:value-of select="@autodeploy"/></xsl:when>
        ...
        <xsl:variable name="maintainer">
            <xsl:choose>
                <xsl:when test="@maintainer"><xsl:value-of select="@maintainer"/></xsl:when>
        ...
        <application name="{@name}" maintainer="{$maintainer}" version="{$version}" auto-deploy="{$autodep}" >
            ...
            <!-- 
                <xsl:call-template name="distinct">
                    <xsl:with-param name="nodes" select="//@role" />
                </xsl:call-template>
             <xsl:for-each select="ms:builder/ms:relation[generate-id(@role) = generate-id(key('roles', 'posrel')[1])]">
             -->
            ...
            <description>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>Install application <xsl:value-of select="@name"/></xsl:otherwise>
                </xsl:choose>
            ...
            <install-notice>
            <xsl:text disable-output-escaping="yes">&lt;![CDATA[</xsl:text> application <xsl:value-of select="@name"/> installed ok. <xsl:text disable-output-escaping="yes">]]&gt;</xsl:text>
            </install-notice>
  ...
  <xsl:template name="reldefs" >
    ...
    <xsl:variable name="directionality">
        <xsl:choose>
            <xsl:when test="/ms:application/ms:reldef[@role=$role and @unidirectional='true']">unidirectional</xsl:when>
    ...
    <reldef source="{$role}" target="{$role}" direction="{$directionality}" builder="{$builder}" guisourcename="{$guiname}" guitargetname="{$guiname}" />
  </xsl:template>
  ...
  -->
    <xsl:template match="ms:builder" mode="application">
    	<xsl:variable name="version"><xsl:call-template name="getbuilderversion"/></xsl:variable>
    	<xsl:variable name="maintainer"><xsl:call-template name="getbuildermaintainer"/></xsl:variable>
    	<builder maintainer="{$maintainer}" version="{$version}"><xsl:value-of select="@name"/></builder>
    </xsl:template>
    ...
    ***********************************************************  -->
                ...
                <builder    name="{@name}" maintainer="{$maintainer}" version="{$version}" extends="{$extends}"  
                    ...
                    <xsl:variable name="builderclass">
                        <xsl:choose>
                            ...
                            <xsl:when test="$defaultbuilderclass"><xsl:value-of select="$defaultbuilderclass"/></xsl:when>
                        </xsl:choose>
                    </xsl:variable>
                    
                    <xsl:if test="not($builderclass='')">
                        <class><xsl:value-of select="$builderclass"/></class>
                        <xsl:call-template name="blankline"/>
                    </xsl:if>
                    
                    <xsl:variable name="searchage">
                        <xsl:choose>
                            <xsl:when test="@searchage"><xsl:value-of select="@searchage"/></xsl:when>
                            <xsl:when test="$defaultbuildersearchage"><xsl:value-of select="$defaultbuildersearchage"/></xsl:when>
                            <xsl:otherwise>3000</xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>
                    
                    <searchage><xsl:value-of select="$searchage"/></searchage>
                    <xsl:call-template name="blankline"/>
                    ...
                    <xsl:variable name="pname">
                        <xsl:choose>
                            <xsl:when test="@pname"><xsl:value-of select="@pname"/></xsl:when>
                            <xsl:otherwise><xsl:value-of select="@name"/></xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>
                    
                    <names>
                        <singular xml:lang="{$lang}"><xsl:value-of select="$sname"/></singular>
                        <plural xml:lang="{$lang}"><xsl:value-of select="$sname"/></plural>
                    </names>
                    
                    <xsl:if test="@description">
                        <descriptions>
                            <descrption><xsl:value-of select="@description"/></descrption>
                        </descriptions>
                    </xsl:if>
    ...
    <xsl:template match="ms:field" mode="builders">
        <field xmlns="http://www.mmbase.org/xmlns/builder">
            ...
            <xsl:variable name="guiname">
                <xsl:choose>
                    <xsl:when test="@guiname"><xsl:value-of select="@guiname"/></xsl:when>
                    <xsl:otherwise><xsl:value-of select="@name"/></xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <gui>
                <guiname xml:lang="{$lang}"><xsl:value-of select="$guiname"/></guiname>
            </gui>
                ...
                <xsl:choose>
                    <xsl:when test="@flags">
                        <xsl:call-template name="stringcontains">
                            <xsl:with-param name="string"><xsl:value-of select="@flags"/></xsl:with-param>
            ...
            <db>
                ...
                <type state="{$state}" size="{$size}" notnull="{$notnull}" key="{$key}"><xsl:value-of select="$type"/></type>
            </db>
        </field>
        <xsl:call-template name="blankline"/>
    </xsl:template>
    ...
    ***********************************************************  -->
    ...
    <xsl:template name="getbuildermaintainer">
            <xsl:choose>
                <xsl:when test="@maintainer"><xsl:value-of select="@maintainer"/></xsl:when>
                <xsl:when test="$defaultmaintainer"><xsl:value-of select="$defaultmaintainer"/></xsl:when>
                <xsl:otherwise>mmbase.org</xsl:otherwise>
            </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="position">
        <xsl:param name="flag"/>
        <xsl:choose>
            <xsl:when test="@input and $flag='i'"><xsl:value-of select="@input"/></xsl:when>
            <xsl:when test="@search and $flag='s'"><xsl:value-of select="@search"/></xsl:when>
            <xsl:when test="@list and $flag='l'"><xsl:value-of select="@list"/></xsl:when>
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="@editor">
                        <xsl:call-template name="stringcontains">
                            <xsl:with-param name="flag"><xsl:value-of select="$flag"/></xsl:with-param>
                            <xsl:with-param name="string"><xsl:value-of select="@editor"/></xsl:with-param>
                            <xsl:with-param name="notfound">-1</xsl:with-param>
                            <xsl:with-param name="found"><xsl:value-of select="position()"/></xsl:with-param>
                        </xsl:call-template>
                    </xsl:when>
    ...
    <xsl:template name="stringcontains">
        ...
        <xsl:choose>
            <xsl:when test="starts-with($string, $flag)"><xsl:value-of select="$found"/></xsl:when>
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="string-length($string) > 1">
                        <xsl:call-template name="stringcontains">
                            ...
                            <xsl:with-param name="notfound"><xsl:value-of select="$notfound"/></xsl:with-param>
                            <xsl:with-param name="found"><xsl:value-of select="$found"/></xsl:with-param>
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise><xsl:value-of select="$notfound"/></xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
