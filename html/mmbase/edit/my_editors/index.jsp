...
%>
	<tr>
	  ...
	  <td><a href="index.jsp?type=<%= nm.getName() %>" title="show nodes"><%= guiName = nm.getGUIName() %></a></td>
	  <td nowrap="nowrap">
	    <a href="index.jsp?type=<%= nm.getName() %>" title="show nodes"><img src="img/mmbase-edit.gif" alt="edit" width="21" height="20" border="0" /></a>
	    <a href="new_object.jsp?type=<%= nm.getName() %>" title="new"><img src="img/mmbase-new.gif" alt="new" width="21" height="20" border="0" /></a>
	  </td>
	</tr>
<% } %>
</table>
...
%>
<mm:listnodes type="$type" constraints="<%= search_str %>">
<mm:first>
...
<mm:fieldlist type="list" nodetype="$type"><% colspan++; %></mm:fieldlist>
<table width="100%" border="0" cellspacing="0" cellpadding="4">
<tr bgcolor="#CCCCCC">
  <td colspan="<%= colspan - 1 %>" class="title-m">
	<%= total_found %> found of type 
	<mm:nodeinfo nodetype="$type" type="guitype" />  (<%= type %>) 
  </td>
  <td>
	<a href="#search" title="search"><img src="img/mmbase-search.gif" alt="search" width="21" height="20" border="0" /></a>
	<a href="new_object.jsp?type=<%= type %>" title="new"><img src="img/mmbase-new.gif" alt="new" width="21" height="20" border="0" /></a>
 </td>
</tr>
<mm:listnodes id="node_number" type="$type" 
	...
	<tr>
	  <td>&nbsp;</td>
	  <mm:fieldlist type="list" nodetype="$type"><td class="name"> <mm:fieldinfo type="guiname" /> </td></mm:fieldlist>
	  <td>&nbsp;</td>
	</tr>
<!-- result rows -->
	</mm:first>
...
<!-- result rows end -->
	...
	%>
		<a href="<mm:url referids="type"><mm:param name="dayofs"><%= dayofs %></mm:param><mm:param name="o"><%= prev %></mm:param><mm:param name="s" value="$search_string" /></mm:url>"><img src="img/mmbase-left.gif" alt="previous" width="21" height="20" border="0" /></a>
	<% 
	}
	if (ofs >= max) {  
	%>
		<a href="<mm:url referids="type"><mm:param name="dayofs"><%= dayofs %></mm:param><mm:param value="0" name="o" /><mm:param name="s" value="$search_string" /></mm:url>"><img src="img/mmbase-up.gif" alt="index" width="21" height="20" border="0" /></a>
	<%
	}
	if (next < total_found) { 
	%>
		<a href="<mm:url referids="type"><mm:param name="dayofs"><%= dayofs %></mm:param><mm:param name="o"><%= next %></mm:param><mm:param name="s" value="$search_string" /></mm:url>"><img src="img/mmbase-right.gif" alt="next" width="21" height="20" border="0" /></a>
	<% } %>
	  </td>
	</tr></table>
	</mm:last>
...
<!-- ### Search form ### -->
<p><a name="search">&nbsp;</a></p>
<form method="post" action="<mm:url referids="type" />">
<table border="0" cellspacing="0" cellpadding="4" class="table-form">
	<tr bgcolor="#CCCCCC">
	  <td colspan="2" class="title-m">Search node of type <mm:nodeinfo nodetype="$type" type="guitype" /> (<%= type %>)</td>
	</tr>
	<tr valign="top">
