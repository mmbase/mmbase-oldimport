...
<mm:import externid="kind"		jspvar="kind" />	<%-- create relation of this kind --%>
	...
	<mm:present referid="nr">
		<tr>
		  <td width="24" align="right">
			<a href="relate_object.jsp?nr=<%= nr %>&amp;ntype=<%= ntype %>&amp;kind=<%= kind %>&amp;dir=<mm:write referid="dir" />" title="search node for new relation"><img src="img/mmbase-search.gif" alt="search node" width="21" height="20" border="0" /></a>
		  </td>
...
<mm:present referid="ntype">
	<mm:notpresent referid="save_it">
		<mm:present referid="nr">
			<form enctype="multipart/form-data" name="create" action="new_object.jsp?ntype=<%= ntype %>&nr=<%= nr %>&kind=<%= kind %>&dir=<%= dir %>" method="post">
		</mm:present>
...
<mm:present referid="nr">		<%-- when there is nr, we assume: kind, role --%>
