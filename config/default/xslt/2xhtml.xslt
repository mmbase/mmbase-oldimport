...
<xsl:stylesheet xmlns:xsl = "http://www.w3.org/1999/XSL/Transform" version = "1.0" >
  ...
  <xsl:param name="formatter_requestcontext"></xsl:param> <!-- this information is needed to correctly construct img.db urls -->
  ...
  <xsl:template match="object[@type='news' and @complete='true']">
  ...
  <xsl:template match="object[@type='news']/field[@name='title']" >
     <h1><xsl:value-of select="." /></h1>
  </xsl:template>

  <xsl:template match="object[@type='news']/field[@name='subtitle']" >
  ...
  <xsl:template match="object" mode="concise">
	  <xsl:choose>
		...
		<img src="{$formatter_requestcontext}/img.db?{./field[@name='number']}" alt="{./field[@name='description']}" align="right"  />
		</xsl:when>
  ...
  <xsl:template match="object" mode="inline">
	<xsl:choose>
		<xsl:when test="@type='images'">
		  <a href="{$formatter_requestcontext}/img.db?{./field[@name='number']}" alt="{./field[@name='description']}">plaatje</a>
		</xsl:when>
		<xsl:when test="@type='urls'">
  ...
  <xsl:template match="p" mode="sub">
	...
	<xsl:apply-templates />	
	<xsl:apply-templates  select="$relatednodes[@type='urls']"  mode="concise" />
  </xsl:template>
