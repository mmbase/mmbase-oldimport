...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="javascript">
    ...
    <xsl:if test="$newfromlist!=&apos;-1&apos;">
      <script type="text/javascript" src="{$javascriptdir}list.jsp{$sessionid}?language={$language}&amp;country={$country}&amp;timezone={$timezone}&amp;referrer={$referrer_encoded}&amp;newfromlist={$newfromlist}">
        <xsl:comment>help IE</xsl:comment>
      </script>
    </xsl:if>
  </xsl:template>
              ...
              <td>
                <form>
                  ...
                  <select name="realsearchfield">
                    ...
                    <!-- hh to delete the "Wijzigen..." prompt in the lists: <xsl:call-template name="search-fields-default" /> -->
                  </select>
