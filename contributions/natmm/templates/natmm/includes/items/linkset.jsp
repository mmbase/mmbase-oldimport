...
<tr><td style="padding:4px;padding-top:14px;">
	<a href="javascript:history.go(-1);" class="subtitle"><bean:message bundle="LEOCMS" key="items.back" /></a>
  ...
  if(!shop_itemId.equals("-1")) {
		...
		%><a href="<mm:url page="<%= ph.createPaginaUrl(paginaID,request.getContextPath()) %>" />" class="subtitle">><bean:message bundle="LEOCMS" key="items.more" />
    <mm:node number="<%= paginaID %>"><mm:field name="title" /></mm:node></a><br><%
		...
		%><mm:node number="<%= shop_itemId %>"
			...
			%></mm:field
		></mm:node
		><a href="<mm:url page="<%= shop_itemHref %>" />" class="subtitle"><bean:message bundle="LEOCMS" key="items.mailafriend" /></a>&nbsp;<a href="<mm:url page="<%= shop_itemHref %>" />"><img src="media/shop/mail.gif" border="0" alt=""></a><br><%
	}
	%>
