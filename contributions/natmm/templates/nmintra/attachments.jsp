    ...
    <tr>
        ...
        <tr><%
            ...
            while(colNumber<=numberOfColumns) { 
                %><td style="width:<%= 100/numberOfColumns %>%;"><%@include file="includes/attachmentstable.jsp" %></td><%
...
%><table cellpadding="0" cellspacing="0" border="0" align="center">
    <tr>
        ...
        <td><div><%
            if(thisOffset>0) { 
                %><a target="_top" href="attachments.jsp<%= templateQueryString  %>&offset=<%= thisOffset-1 %>">[<<- vorige ]</a>&nbsp;&nbsp;<%
            }for(int i=0; i < (listSize/10 + 1); i++) { 
                if(i==thisOffset) {
                    %><%= i+1 %>&nbsp;&nbsp;<%
                } else { 
                    %><a target="_top" href="attachments.jsp<%=  templateQueryString  %>&offset=<%= i %>"><%= i+1 %></a>&nbsp;&nbsp;<%
                } 
            }
            if(thisOffset+1<(listSize/10 + 1)) { 
                %><a  target="_top" href="attachments.jsp<%= templateQueryString %>&offset=<%= thisOffset+1 %>">[volgende ->>]</a><%
            } 
        %></div>
        </td>
    </tr>
