...
public class OptionedStats {
   ...
   private NodeList filterOnUser(Cloud cloud, NodeList nl, String sNodeNumber, String listtype){
      if(listtype.equals("Bestelwijze")&&!sNodeNumber.equals("")) {
         NodeList userNl = null;
         String thisInschrijving = "";
         int i = 0;
         while(i < nl.size()) {
            thisInschrijving = nl.getNode(i).getStringValue("inschrijvingen.number");
            userNl = cloud.getList(thisInschrijving,"inschrijvingen,schrijver,users","users.number","users.number = '" + sNodeNumber + "'",null,null,"destination",false);
            if(userNl.isEmpty()) { 
               nl.remove(i);
            } else {
               i++;
   ...
   private int getCounts(Cloud cloud, String sNodeNumber, String evenementTimeConstraint, String listtype, String statstype){
      ...
      if (statstype.equals("inschrijvingen")){
         nl = cloud.getList((listtype.equals("Bestelwijze") ? "" : sNodeNumber),
            sRealNodepath,"inschrijvingen.number",sRealConstraints,null,null,null,false);
         nl = filterOnUser(cloud,nl,sNodeNumber,listtype);
         iResult += nl.size();
      } else if (statstype.equals("deelnemers")){
         sRealNodepath += ",posrel2,deelnemers";
         nl = cloud.getList((listtype.equals("Bestelwijze") ? "" : sNodeNumber),
            sRealNodepath,"inschrijvingen.number,deelnemers.bron",sRealConstraints,null,null,null,false);
         nl = filterOnUser(cloud,nl,sNodeNumber,listtype);
         ...
         sRealNodepath += ",posrel2,deelnemers";
         nl = cloud.getList((listtype.equals("Bestelwijze") ? "" : sNodeNumber),
            sRealNodepath,"inschrijvingen.number,posrel2.pos",sRealConstraints,null,null,null,false);
         nl = filterOnUser(cloud,nl,sNodeNumber,listtype);
         for(int i = 0; i < nl.size(); i++) {
         ...
         nl = cloud.getList((listtype.equals("Bestelwijze") ? "" : sNodeNumber),
            sRealNodepath,"inschrijvingen.number,evenement.number",sRealConstraints,null,null,null,false);
         nl = filterOnUser(cloud,nl,sNodeNumber,listtype);
