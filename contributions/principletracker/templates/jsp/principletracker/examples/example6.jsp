...
<HEAD>
<mm:cloud>
   <TITLE>Search Query Bridge Example 6</TITLE>
</HEAD>
  <hr>
   ...
   example6.html, same as example 4 but adds principlenumber and sorts on them. 
  <hr>
   <pre>
    ...
    StepField field4 = query.addField(step3, principlemanager.getField("principlenumber"));
    ...
    // we want to order them based on principlenumber
    query.addSortOrder(field4, SortOrder.ORDER_ASCENDING);
  ...
  <%
    ...
    while (i.hasNext()) {
      ...
      %>
      <b>output=</b><%=node.getStringValue("principle.principlenumber")%> <%=node.getStringValue("principle.qualification")%> - <%=node.getStringValue("principlesets.name")%> - <%=node.getStringValue("principle.name")%><br />
     <%
    }
  %>
  <hr>
