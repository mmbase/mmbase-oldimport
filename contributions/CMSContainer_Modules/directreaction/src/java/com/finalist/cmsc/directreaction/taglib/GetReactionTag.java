...
/**
 * The GetReactionTag will retrieve a single reaction node from the live database
 * and then populate and return a Reaction object.
 * 
 * @author jderuijter
 *
 */
public class GetReactionTag extends SimpleTagSupport {
   ...
   public void doTag() throws JspException, IOException {
      ...
      Reaction reaction = 
          new Reaction(node.getIntValue("number"), 
                  node.getStringValue("title"), 
                  node.getStringValue("body"), 
                  node.getStringValue("name"), 
                  node.getStringValue("email"),
                  node.getDateValue("creationdate"),
                  this.getRelatedContentTitle());
   ...
   public String getRelatedContentTitle() {
       ...
       NodeList nodeList = getLiveCloud().getList(""+number, "reaction,contentelement", "contentelement.title", null, null,null,null,true);
