...
public class PageWizardForm extends ActionForm {
	...
	protected void loadDefinition(Cloud cloud) {
		...
		definition = new PageWizardDefinition(wizardNode.getNumber(), wizardNode.getStringValue("name"), wizardNode.getStringValue("description"));
		...
		for(NodeIterator pni = portletList.nodeIterator(); pni.hasNext();) {
			...
			for(NodeIterator cni = choiceList.nodeIterator(); cni.hasNext();) {
				...
				PageWizardChoice choice = new PageWizardChoice(choiceNode.getStringValue("type"), choiceNode.getStringValue("parameter"));
				choices.add(choice);
			}
