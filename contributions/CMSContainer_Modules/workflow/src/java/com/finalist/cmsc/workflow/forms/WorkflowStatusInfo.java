    ...
    public WorkflowStatusInfo(NodeList statusList) {
        for (Iterator iter = statusList.iterator(); iter.hasNext();) {
            ...
            if ("content".equals(type)) {
                if (Workflow.STATUS_DRAFT.equals(status)) contentDraft = count;
                if (Workflow.STATUS_FINISHED.equals(status)) contentFinished += count;
                ...
                if (Workflow.STATUS_PUBLISHED.equals(status)) contentPublished = count;
            }
            if ("link".equals(type)) {
