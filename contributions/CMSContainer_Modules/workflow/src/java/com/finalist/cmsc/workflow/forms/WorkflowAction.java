...
public abstract class WorkflowAction extends MMBaseFormlessAction {
	...
	public ActionForward execute(ActionMapping mapping,
		...
		if (!StringUtil.isEmpty(actionValueStr)) {
			...
			List<Node> workflowErrors = performWorkflowAction(actionValueStr,
					nodes, remark);

			if (workflowErrors != null && workflowErrors.size() > 0) {
				String url = mapping.getPath() + "?status="
		...
		if (StringUtil.isEmpty(laststatus) || laststatus == null) {
			...
			request.setAttribute("laststatus",
					laststatus.equals("true") ? false : true);
		}
		...
		Queries.addConstraint(listQuery, WorkflowManager.getStatusConstraint(
				listQuery, status));
		...
		Queries.addConstraint(listQuery, WorkflowManager.getTypeConstraint(
				listQuery, type));
		NodeQuery wfQuery = createDetailQuery(cloud, orderby,
				(laststatus == null) ? false : (laststatus.equals("true")));

		addWorkflowListToRequest(request, cloud, wfQuery, listQuery, "results");

		request.setAttribute("acceptedEnabled", Workflow
				.isAcceptedStepEnabled());
	...
	private void addWorkflowListToRequest(HttpServletRequest request,
			Cloud cloud, NodeQuery detailQuery, NodeQuery listQuery,
			String attributeName) {
		NodeList ceWorkflowNumbers = cloud.getList(listQuery);
		if (!ceWorkflowNumbers.isEmpty()) {
			NodeQuery numbersQuery = createDetailsWithNumbersQuery(detailQuery,
					ceWorkflowNumbers);
	...
	private NodeQuery createDetailsWithNumbersQuery(NodeQuery wfQuery,
			NodeList workflowNumbers) {
		NodeQuery detailQuery = (NodeQuery) wfQuery.clone();

		BasicStep wfStep = (BasicStep) detailQuery
				.getStep(WorkflowManager.WORKFLOW_MANAGER_NAME);
		...
		while (wfIterator.hasNext()) {
			...
			int workflowNumber = node
					.getIntValue(WorkflowManager.WORKFLOW_MANAGER_NAME
							+ ".number");
	...
	protected void addOrderBy(NodeManager manager, NodeQuery query,
			String fieldname, boolean aord) {
