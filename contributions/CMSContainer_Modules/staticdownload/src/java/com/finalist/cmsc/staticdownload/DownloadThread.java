...
public class DownloadThread extends Thread {
   ...
   private void setupSuffix() throws IOException{
      InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream("../config/subfix.txt");
   ...
   public void run() {
     try {
         cleanUp();
         download();
         File file = new File(downloadSettings.getStorePath());//get the files which wget last download
         setupSuffix();
         modifyDownloadPath();
         findAssociatdFiles(file, new MyFilenameFilter(".css", ".js", ".html"));
    ...
    * @throws IOException */
   private void findAssociatdFiles(File file , MyFilenameFilter myfilter) throws IOException {
      ...
      for (int i = 0; i < files.length; i++) {
         if (files[i].isFile()) {
            for(String suf:suffix){
               String outputData = this.getTargetStringAndWriteFile(files[i],suf);
               if(outputData.length()!=0 && files[i].length()!=outputData.length()){
                  try {
    ...
    */
   private String downloadAndModifyPath(String targetStringNew,String tempString,File file){
      String downloadPath;
      if(StringUtils.isEmpty(webappName)){
         downloadPath = webPath+targetStringNew.substring(1);
      }else{
         downloadPath = webPath+targetStringNew.substring(webappName.length()+2);
      }
      if(!redownloadfiles.contains(downloadPath))
         redownloadfiles.add(downloadPath);
      String filePath = file.getParent();
      int beginIndex = filePath.indexOf(getUrl());
      String filePathIntercept = filePath.substring(beginIndex+getUrl().length());
      ...
      if(StringUtils.isEmpty(filePathIntercept)&&StringUtils.isEmpty(webappName)){
         ...
         tempString = this.regularReplace(tempString, targetStringNew, targetStringNews);         
      }else if(StringUtils.isNotEmpty(filePathIntercept)&&StringUtils.isEmpty(webappName)){
         tempString = this.regularReplace(tempString, targetStringNew, docs+targetStringNew.substring(1));
      }else if(StringUtils.isEmpty(filePathIntercept)&&StringUtils.isNotEmpty(webappName)){
         String targetStringNews =targetStringNew.substring(webappName.length()+2);
         tempString = this.regularReplace(tempString, targetStringNew, targetStringNews);            
      }else if(StringUtils.isNotEmpty(filePathIntercept)&&StringUtils.isNotEmpty(webappName)){
         tempString = this.regularReplace(tempString, "/"+webappName+"/", docs);
      }
      return tempString;
   }
    ...
    * @throws IOException 
    */
   private String getTargetStringAndWriteFile(File file,String suffix) throws IOException{
      BufferedReader reader = null;
      String outputData = "";
         try{
            reader = new BufferedReader(new FileReader(file));
            String tempString = null;
               while((tempString = reader.readLine())!=null&&!file.getName().endsWith(".html")){
                  ...
                  if(targetPath!=""){
                     ...
                     if(StringUtils.isNotEmpty(webappName)&&(tagStart = fileParentPath.indexOf(webappName))>=0&&fileParentPath.contains("js")){
                        int tagEnd = fileParentPath.indexOf("\\js",tagStart+webappName.length()+1);
                        if(tagEnd>=0){
                           directoryName = fileParentPath.substring(tagStart+webappName.length()+1, tagEnd);
                           this.spellPath(targetPath,directoryName);
                        }
                     }else if(StringUtils.isNotEmpty(webappName)&&(tagStart =fileParentPath.indexOf(webappName))>=0&&fileParentPath.contains("css")){
                        int tagEnd = fileParentPath.indexOf("\\css",tagStart+webappName.length()+1);
                        if(tagEnd>=0){
                           directoryName = fileParentPath.substring(tagStart+webappName.length()+1, tagEnd);
                           this.spellPath(targetPath,directoryName);
                        }
                     }else if(StringUtils.isEmpty(webappName)&&fileParentPath.contains("css")){
                        addDownloadPath("css",file,targetPath);
                     }else if(StringUtils.isEmpty(webappName)&&fileParentPath.contains("js")){
                        addDownloadPath("js",file,targetPath);
                     }
                  }
               }
               while((tempString = reader.readLine())!=null&&file.getName().endsWith(".html")){
                  String targetString = regularJudge("(?m)^\\s*var.*"+suffix+".*$", tempString);
                  if(StringUtils.isNotEmpty(targetString)){                     
                     String targetStringNew = regularJudge("[\\w/\\.-]+\\."+suffix,targetString);
                     if(StringUtils.isNotEmpty(targetStringNew)){
                        if(targetStringNew.startsWith("/"+webappName)&&StringUtils.isNotEmpty(webappName)){
                           tempString = this.downloadAndModifyPath(targetStringNew, tempString, file);
                        }else if(targetStringNew.startsWith("/")&&StringUtils.isEmpty(webappName)){
                           tempString = this.downloadAndModifyPath(targetStringNew, tempString, file);
                        }
                     }
                  }
                  String str = regularJudge("[\\s.]*background-image:\\s*url.*", tempString);
                  if(StringUtils.isNotEmpty(str)){
                     ...
                     if(StringUtils.isNotEmpty(targetName)){
                        if(targetName.startsWith("/"+webappName)&&StringUtils.isNotEmpty(webappName)){
                           tempString = this.downloadAndModifyPath(targetName, tempString, file);
                        }else if(targetName.startsWith("/")&&StringUtils.isEmpty(webappName)){
                           tempString = this.downloadAndModifyPath(targetName, tempString, file);
                        }
                     }
                  }
