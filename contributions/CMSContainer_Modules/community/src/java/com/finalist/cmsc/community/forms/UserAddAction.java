...
public class UserAddAction extends AbstractCommunityAction {
	...
	public ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest httpServletRequest,
		...
		if (!isCancelled(httpServletRequest)) {
			if (null==groupFoeward||",".equals(groupFoeward)) {
				...
				authId = updatePersonRecord(errors, authId, userForm,
						accountName, as, ps);
			}else{
			...
			if (null==groupFoeward||",".equals(groupFoeward)) {
				return setActionForward(actionMapping, httpServletRequest,
						tmpForward);
			}else{
				setGroupParamater(httpServletRequest, groupFoeward);
				return actionMapping.findForward(FORWARD_GROUP);
		...
		if (errors.size() > 0) {
			...
			if ("newslettersubscribers".equals(tmpForward)) {
				...
				ActionForward ret;
				if (((UserForm) actionForm).getAction().equalsIgnoreCase(
						UserForm.ACTION_ADD)) {
					ret = new ActionForward(actionMapping.findForward(
				...
				String path = (String) httpServletRequest.getSession()
						.getAttribute("path");
				path = path.substring(0, path.indexOf("page") - 1) + "&"
	...
	private ActionForward setActionForward(ActionMapping actionMapping, HttpServletRequest httpServletRequest, String tmpForward) {
		log.info("cancelled");
		ActionForward ret;
	...
	private Long updatePersonRecord(ActionMessages errors, Long authId, UserForm userForm, String accountName, AuthenticationService as, PersonService ps) {
