...
/**
 * This class provides a default framework for Didactor relations, where components
 * can register themselves for events on the builders.
 * @author Johannes Verelst &lt;johannes.verelst@eo.nl&gt;
 */
public class DidactorRel extends InsRel {
    private static Logger log = Logging.getLoggerInstance(DidactorRel.class.getName());
    ...
    public MMObjectNode getSource(MMObjectNode rel) {
       int snumber = rel.getIntValue("snumber");
       return getNode(snumber);
    }

    public MMObjectNode getDestination(MMObjectNode rel) {
       int dnumber = rel.getIntValue("dnumber");
       return getNode(dnumber);
    }
