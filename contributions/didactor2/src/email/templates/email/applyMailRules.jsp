...
<mm:node number="$user">
  <mm:relatednodes type="mailboxes" constraints="m_type=0" max="1">
	...
	<mm:list nodes="$inbox" path="mailboxes,related,emails" constraints="emails.m_type=2 AND LOCATE(LOWER('$rule'),LOWER(emails.subject)) > 0">
	...
	<mm:field write="false" id="destinationbox" name="mailboxes2.number"/>
	<mm:list nodes="$inbox" path="mailboxes,related,emails" constraints="emails.m_type=2 AND LOCATE(LOWER('$rule'),LOWER(emails.m_from)) > 0">
	    <mm:field write="false" id="relation" name="related.number"/>
	    <mm:field write="false" id="mail" name="emails.number"/>
	    <mm:deletenode number="$relation" deleterelations="false"/>
	    <mm:createrelation role="related" source="destinationbox" destination="mail"/>
