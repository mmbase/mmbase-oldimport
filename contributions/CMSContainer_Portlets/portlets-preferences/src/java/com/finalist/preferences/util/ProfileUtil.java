...
public class ProfileUtil {
  ...
  public static void addPreferences(List<PreferenceVO> preferences){
     ...
     for(PreferenceVO preference : preferences) {
        List<String> values =  ps.getPreferenceValues(preference.getModule(), getUserIdByAuthenticationId(Long.parseLong(preference.getAuthenticationId())), preference.getKey());
        if(values == null || values.size() == 0) {
           ps.createPreference(preference.getModule(), getUserIdByAuthenticationId(Long.parseLong(preference.getAuthenticationId())), preference.getKey(),preference.getValue());
        }
        else {
          ps.updatePreference(preference.getModule(), getUserIdByAuthenticationId(Long.parseLong(preference.getAuthenticationId())), preference.getKey(),values.get(0),preference.getValue());
        }
     }
  }
