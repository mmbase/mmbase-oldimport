...
public class EmailAlertCronJob implements CronJob {
    ...
    private void sendEmailAlerts() {
        ...
        for (int i = 0; i < articles.size(); i++) {
        	...
        	for (int j = 0; j < pages.size(); j++) {
        		...
        		NodeList subscribers = SearchUtil.findRelatedNodeList(cloud.getNode(page.getPageNumber()), "subscriber", "subscriberel", "valid", true); 
	...
	private void sendEmails(Cloud cloud, Map<String, Set<PageInfo>> alerts, String liveHost) {
		...
		if (indexBeginUrlTemplate > -1 && indexEndUrlTemplate > -1) {
			...
			urlTemplate = emailTemplate.substring(indexBeginUrlTemplate + BEGINTEMPLATE.length(), indexEndUrlTemplate - 1).trim();
	...
	private String processTemplate(Set<PageInfo> pages, String beginEmailBody, String urlTemplate, String endEmailBody, String emailAddress, String liveHost) {
		...
		for (PageInfo page: pages) {
			...
			urlFragment = urlFragment.replaceAll(AFMELDENPAGINA, String.format(unsubscribePageTemplate, page.getPageNumber(), emailAddress));
