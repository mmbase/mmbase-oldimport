...
public class EmailAlertPortlet extends ContentPortlet {
        ...
        if (action == null) {
            ...
            if (contentElement != null) { 
                ...
                if (errorMessages.size() == 0) {                 	
                	...
                	//check if the relation between page and subscriber exists, otherwise create it                		
                	NodeManager relationNodeManager = cloud.getNodeManager(SUBSCRIBEREL);
                	if (!existsRelation(relationNodeManager, Integer.parseInt(subscribePage), subscriberNode.getIntValue("number")) ) {
    ...
    private boolean sendConfirmationEmail(Cloud cloud, Node subscriberNode, Node emailAlert, ActionRequest request) {
    	...
    	try {
    		EmailUtil.send(cloud, null, subscriberNode.getStringValue(EMAILADDRESS), fromName, fromEmailAddress, subject, message);
    		sent = true;
    	}
    	catch (Exception e){
	...
	//TODO remove this and use the RelationUtil.exists(...) instead, when the new mmapps is built
	private static boolean existsRelation(NodeManager builder, int source, int destination) {
        ...
        Constraint composite = query.createConstraint(
                s, CompositeConstraint.LOGICAL_AND, d);
