...
/**
 * Do some html body parse.
 * 
 * @author Nikko
 */
public class HtmlBodyParser {
   
   private static Logger log = Logging.getLoggerInstance(HtmlBodyParser.class.getName());

   public static String html2text(String orhtml) {
      String html = "";
      int i = orhtml.indexOf("<body");
      html = orhtml.substring(i, orhtml.length());
...
class Token {
   ...
   private void parseText(boolean previousIsPre) {
      if (type == TOKEN_TAG) {
         ...
         else if (compareTag(TAG_PRE, cs)) isPre = true;
         else if (compareTag(TAG_HR, cs)) text = "\n--------\n";
         else if (compareString(END_TAG_TD, cs)) text = "\t";
         else if (compareString(END_TAG_TR, cs) || compareString(END_TAG_LI, cs)) text = "\n";
      }
      // text token:
