...
<#macro moderationImages>
	...
	function deleteTopic() {
		if (confirm("${I18n.getMessage("Moderation.ConfirmDelete")}")) {
			todo("topicRemove");
			// jahia fork: dont encode returnURL
			document.formModeration.returnUrl.value = "${contextPath}/forums/show/${topic.forumId}";
					...
					<select name="select">
						...
						<#list allCategories as category>
							...
							<#list category.getForums() as forum>
								<option onclick="document.location = '${JForumContext.encodeURL("/forums/show/${forum.id}")}'" value="${forum.id}">${forum.name}</option>
							</#list>
						</#list>
					</select>
					&nbsp;
