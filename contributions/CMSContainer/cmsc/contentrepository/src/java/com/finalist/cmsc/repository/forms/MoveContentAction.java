...
public class MoveContentAction extends MMBaseAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        if (swappedNodenumber > 0) {
            ...
            String remark = resources.getMessage(locale,
                    "content.move.workflow.message", elementNode.getNodeManager().getName(),
                    elementNode.getStringValue("title"));
        ...
        String path = mapping.findForward(SUCCESS).getPath() + "?" + PARAMETER_CHANNEL + "="
    ...
    private int moveDown(int number, int channel) {
        ...
        NodeList nodeList = cloud.getList("" + channel, "contentchannel,contentrel,contentelement",
                "contentrel.number,contentrel.pos,contentelement.number", null, "contentrel.pos",
                "down", null, false);
        ...
        for (NodeIterator ni = nodeList.nodeIterator(); ni.hasNext();) {
            ...
            if (nodeNumber == number) {
                if (ni.hasNext()) { // it could already be the last item in the list
                    ...
                    swap(nextNode.getIntValue("contentrel.number"), node
                            .getIntValue("contentrel.number"));
