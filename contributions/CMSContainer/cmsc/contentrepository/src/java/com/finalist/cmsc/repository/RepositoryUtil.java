...
public class RepositoryUtil {
   ...
   public static NodeQuery createLinkedContentQuery(Node channel, List<String> contenttypes, String orderby,
      ...
      if (year != -1 || month != -1 || day != -1) {
         ...
         if (year != -1) {
            SearchUtil.addConstraint(query, new BasicFieldValueDateConstraint(basicStepField, new Integer(year),
                  FieldValueDateConstraint.YEAR));
         }
         if (month != -1) {
            SearchUtil.addConstraint(query, new BasicFieldValueDateConstraint(basicStepField, new Integer(month),
                  FieldValueDateConstraint.MONTH));
         }
         if (day != -1) {
            SearchUtil.addConstraint(query, new BasicFieldValueDateConstraint(basicStepField, new Integer(day),
   ...
   /**
    * Makes sure the supplied contentelement will have at least one content and
    * one creation channel. If the node has to little, it will relate to the
    * trash contentchannel.
    * 
    ...
    * channel. If the node has none, it will relate to the trash channel.
    * 
    * @param contentelement
   ...
   public static UserRole getRole(Node group, Node channel, boolean rightsInherited) {
      // retrieve a TreeMap where the channels (keys) are ordered on level and
      // path
   ...
   public static RolesInfo getRolesInfo(Cloud cloud, Node group) {
      ...
      TreeMap<String, UserRole> channelsWithRole = SecurityUtil.getRoleMap(treeManagers, CHILDREL, fragmentFieldnames,
            group);
      for (String path : channelsWithRole.keySet()) {
         Node channel = getChannelFromPath(cloud, path);
         info.expand(channel.getNumber());
   ...
   /**
    * This is the method for a USER, the old ones want a GROUP... (even although
    * the are called getRoleForUser(..)
    * 
    * @param channel
