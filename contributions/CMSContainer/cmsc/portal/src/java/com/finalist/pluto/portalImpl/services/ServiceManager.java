 ...
 */
package com.finalist.pluto.portalImpl.services;
...
/**
 * Manages the life-time of services registered during servlet startup. There
 * are two types of services: Services required by the container, and Services
 * used for the portal driver configuration and execution. All services required
 * by the container have to derive from
 * {@link org.apache.pluto.services.ContainerService} and implement the <CODE>init()</CODE>
 * and <CODE>destroy()</CODE> methods as appropriate. All services must derive
 * from the {@link org.apache.pluto.portalImpl.services.Service} interface if
 * they are to be used with the portal driver.
 * <P>
 * By registering the service and its implementation in the file <CODE>/config/services.properties</CODE>,
 * the service will become available to the portal engine. The format of the
 * file is simple:
 * 
 * <PRE>
 * 
 ...
 * </PRE>
 * 
 * Each entry represents one service. The left-hand side is the abstract service
 * class, the right-hand side is the implementation of this service. The
 * services are initialized in the order of appearance.
 * <P>
 * Each service can have its own configuration file, located in <CODE>/config/services</CODE>.
 * It has to have the name of either implementation or abstract class of the
 * service, without the leading package name. For example, the service manager
 * looks for <CODE>LoggerImpl .properties</CODE>. This allows a special
 * implementation to provide different configuration than the general (abstract)
 * service requires.
 * <P>
 * If present, one of the services configuration files is loaded and passed to
 * the service as {@link org.apache.pluto.portalImpl.util.Properties} object.
 * Not providing a service configuration file is okay too, in that case the
 * properties are emptyemptySessionPath .
 * 
...
public class ServiceManager {
	 ...
	 * @exception Exception * if loading <CODE>services.properties</CODE> * or
	 *            initializing any of its contained services fails
	 */
	 ...
	 * 
	 * @param aConfig * the servlet configuration *
	 * @param aServiceConfigFile * The location of <CODE>services.properties</CODE>
	 *        (relative to classpath) *
	 * @param aServiceConfigDir * The direcory with the services' properties
	 *        files (relative to classpath) * *
	 * @exception Exception * if loading <CODE>services.properties</CODE> * or
	 *            initializing any of its contained services fails
	 */
	public static void init(ServletConfig aConfig, String aServiceConfigFile, String aServiceConfigDir) throws Exception {
		...
		log.info("ServiceManager: Loading services...");
		...
		try {
			...
			log.error("ServiceManager: File \"" + aServiceConfigFile + "\" cannot be found or read.");
		...
		for (Iterator iter = props.names(); iter.hasNext();) {
			...
			try {
				...
				log.error("ServiceManager: A service with name " + serviceBaseName + " cannot be found.");
			...
			try {
				...
				log.info(StringUtils.nameOf(serviceBase) + " initializing...");

				service.init(aConfig, serviceProps);

				log.info(StringUtils.nameOf(serviceBase) + " done.");
			} catch (ClassNotFoundException exc) {
				log.error("ServiceManager: A service implementation with name " + serviceImplName + " cannot be found.", exc);

				continue;
			} catch (ClassCastException exc) {
				log.error(
						"ServiceManager: Service implementation " + serviceImplName + " is not a service of the required type.",
						exc);

				continue;
			} catch (InstantiationException exc) {
				log.error("ServiceManager: Service implementation " + serviceImplName + " cannot be instantiated.", exc);

				continue;
			} catch (Exception exc) {
				log.error("ServiceManager: An unidentified error occurred", exc);
		...
		log.info("ServiceManager: Services initialized (" + numSuccessful + "/" + numAll + " successful).");
	...
	public static void destroy(ServletConfig aConfig) {
		...
		for (Iterator iterator = cServicesList.iterator(); iterator.hasNext();) {
			...
			try {
				...
				log.error("ServiceManager: Service couldn't be destroyed.", exc);
			}
		}
	...
	public static void hotInit(ServletConfig aConfig, String aServiceConfigFile, String aServiceConfigDir, String theService)
			throws Exception {
		// avoid duplicate initialization of services
		...
		log.info("ServiceManager: HOTHOT Loading service: " + theService);
		...
		for (Iterator iter = props.names(); iter.hasNext();) {
			...
			if (serviceBaseName.equals(theService)) {
				...
				try {
					...
					log.error("ServiceManager: Service implementation " + serviceImplName
							+ " is not a service of the required type.", exc);
					continue;
				} catch (InstantiationException exc) {
					log.error("ServiceManager: Service implementation " + serviceImplName + " cannot be instantiated.", exc);
					continue;
		...
		log.error("ServiceManager: Services initialized (" + numSuccessful + "/" + numAll + " successful).");
		if (numSuccessful != numAll) {
			throw new Exception("ServiceManager: Services initialized (" + numSuccessful + "/" + numAll + " successful).");
		}
	}
