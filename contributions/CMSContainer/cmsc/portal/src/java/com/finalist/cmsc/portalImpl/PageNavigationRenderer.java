...
public class PageNavigationRenderer implements NavigationItemRenderer {
	...
	public void render(NavigationItem item, HttpServletRequest request, HttpServletResponse response, ServletContext servletContext, ServletConfig sc, PortalRegistry registry) {
		if(item instanceof Page) {
			...
			throw new IllegalArgumentException("Got a wrong type in the PageNavigationRenderer (only wants Page or derived classes), was"+item.getClass());
		}	
	}
	...
	public TreeElement getTreeElement(NavigationInformationProvider renderer, Node parentNode, NavigationItem item, TreeModel model) {
		...
		if (SecurityUtil.isEditor(role)) {
	        element.addOption(renderer.createOption("edit_defaults.png", "site.page.edit", "PageEdit.do?number=" + parentNode.getNumber()));
	        element.addOption(renderer.createOption("new.png", "site.page.new", "PageCreate.do?parentpage=" + parentNode.getNumber()));

	        if(ModuleUtil.checkFeature(FEATURE_RSSFEED)) {
	        	element.addOption(renderer.createOption("rss_new.png", "site.rss.new", "../rssfeed/RssFeedCreate.do?parentpage=" + parentNode.getNumber()));
	        }

            if (SecurityUtil.isWebmaster(role) || (model.getChildCount(parentNode) == 0 && !Publish.isPublished(parentNode))) {
	        	element.addOption(renderer.createOption("delete.png", "site.page.remove", "PageDelete.do?number=" + parentNode.getNumber()));
            }

	        if (NavigationUtil.getChildCount(parentNode) >= 2) {
	        	element.addOption(renderer.createOption("reorder.png", "site.page.reorder", "reorder.jsp?parent=" + parentNode.getNumber()));
	        }
		
			if (SecurityUtil.isChiefEditor(role)) {
	        	element.addOption(renderer.createOption("cut.png", "site.page.cut", "javascript:cut('" + parentNode.getNumber() + "');"));
	        	element.addOption(renderer.createOption("copy.png", "site.page.copy", "javascript:copy('" + parentNode.getNumber() + "');"));
	        	element.addOption(renderer.createOption("paste.png", "site.page.paste", "javascript:paste('" + parentNode.getNumber() + "');"));
			}

            if(ModuleUtil.checkFeature(FEATURE_PAGEWIZARD)) {
	        	element.addOption(renderer.createOption("wizard.png", "site.page.wizard", "../pagewizard/StartPageWizardAction.do?number=" + parentNode.getNumber()));
            }

			if (SecurityUtil.isWebmaster(role) && ModuleUtil.checkFeature(FEATURE_WORKFLOW)) {
	        	element.addOption(renderer.createOption("publish.png", "site.page.publish", "../workflow/publish.jsp?number=" + parentNode.getNumber()));
	        	element.addOption(renderer.createOption("masspublish.png", "site.page.masspublish", "../workflow/masspublish.jsp?number=" + parentNode.getNumber()));
			}
		}
    	element.addOption(renderer.createOption("rights.png", "site.page.rights", "../usermanagement/pagerights.jsp?number=" + parentNode.getNumber()));
