...
public class FactoryManagerServiceImpl extends FactoryManagerService {
	 ...
	 * @exception Exception * if initializing any of the factories fails
	 */
	protected void init(ServletConfig config, Properties aProperties) throws Exception {
		...
		for (Iterator iter = factoryImpls.keySet().iterator(); iter.hasNext();) {
			...
			try {
				...
				log.error("FactoryManager: A factory implementation with name " + factoryImplName + " cannot be found.", exc);
				throw exc;
			} catch (ClassCastException exc) {
				log.error(
						"FactoryManager: Factory implementation " + factoryImplName + " is not a factory of the required type.",
						exc);
				throw exc;
