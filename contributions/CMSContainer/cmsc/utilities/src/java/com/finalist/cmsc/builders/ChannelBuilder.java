...
public abstract class ChannelBuilder extends MMObjectBuilder {
   ...
   public int insert(String owner, MMObjectNode node) {
      // don't use this method to add stuff to the channelCache in the
      // ChannelUtil.
   ...
   public boolean commit(MMObjectNode objectNode) {
      ...
      if (objectNode.getChanged().contains(fragmentFieldname)) {
         ...
         TreePathCache
               .updateCache(getTableName(), objectNode.getNumber(), objectNode.getStringValue(fragmentFieldname));
      }
   ...
   /**
    * called when a value is set. used to store the old value for compare in the
    * commit()
    */
   @Override
   ...
   /**
    * Executes a function on the field of a node, and returns the result. In
    * case the function equals "url" the full url including domain name and
    * application root is returned.
    * 
    * @param node
    *           the node whose fields are queries
    * @param field
    *           The function to execute
    * @return the result of the 'function', or null if no valid functions could
    *         be determined
    */
   @Override
   ...
   private String getPath(int number) {
      // reduce creating useless cloud objects, because this code is executed in
      // the core of MMBase
      ...
      if (getTableName().equals(event.getRelationDestinationType()) && relationNumber == event.getRole()) {
         ...
         switch (event.getType()) {
            case Event.TYPE_DELETE:
               ...
               if (path != null) {
                  log.debug("delete cut action " + destination + " " + path
                        + " (this should happen after the new relation)");
               }
               break;
            case Event.TYPE_NEW:
