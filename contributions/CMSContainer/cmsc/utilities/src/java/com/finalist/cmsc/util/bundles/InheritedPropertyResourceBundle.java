 ...
 * inheritance.
 *
 * <p>The inheritance relationships can be defined
 * in-line, or they can be provided in a relationships file which
 * is locatable by the given ClassLoader.  This file must be
 * named <resource_base_name>.relationships.  The format for this
 * file is simply the list of parent bundles in the order of
 * precedence.  Comments (lines starting with the '#' character,)
 * and blank lines are permitted.
 *
 *<p><strong>Example:</strong><br>
 * <pre> # Relationships file for the resources.ProductNames bundle.
 * # resources/ProductNames.relationships
 *
 * resources.CompanyStandards
 * resources.Products</pre>
 *
 *<p>In the above example, if a key is searched for that does not
 * exist in the backed PropertyResourceBundle, then the 
 * resources.CompanyStandards bundle is checked first.  If it is not
 * found there, the resources.Products bundle is checked next.
 *
...
public class InheritedPropertyResourceBundle extends ResourceBundle {
   ...
   /**
    * The locale for this resource bundle.  This is optional.  If no
    * locale is specified then the default locale is used.
    */
   private Locale locale = null;

   /**
    * The ClassLoader used to look for resource bundles.  This is optional.
    * If no ClassLoader is specified, then the system class loader is used.
    */
   private ClassLoader loader = null;
   
   /**
    * Creates a new InheritedPropertyResourceBundle.  This constructor
    * should only be called by a resource bundle manager so that
    * duplicate versions are not created.
    * @param baseName the base name of the resource bundle.
    * @param locale the locale for which the resource bundle will be loaded.
    * This is optional.  If no locale is provided, the default locale is
    * used.
    * @param loader the ClassLoader which is used to find the resource
    * bundle and relationships files.
    */
   ...
   /**
    * Returns an enumeration of keys this resource bundle knows about.
   ...
   /**
    * Gets an object from this resource bundle.  If the resource is
    * not found in the backing PropertyResourceBundle, then parent
    * bundles are searched, in order, for the resource.
    ...
    * @param r resource bundle to remove
    */
