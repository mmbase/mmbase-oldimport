...
public class CombinedResourceBundle extends ResourceBundle {
    ...
    protected Object handleGetObject(String key) {
        ...
        for (Iterator iter = bundles.iterator(); iter.hasNext();) {
            ResourceBundle rb = (ResourceBundle) iter.next();
            try {
                retVal = rb.getObject(key);
            }
            catch (MissingResourceException mre) {
                // Do nothing here. This will cause a search in the following bundles.
            }
            if (retVal != null) { 
                return retVal; 
            }
    ...
    public Enumeration<String> getKeys() {
        ...
        for (Iterator iter = bundles.iterator(); iter.hasNext();) {
            ...
            for (Enumeration g = rb.getKeys(); g.hasMoreElements();) {
    ...
    /**
     * Adds the ResourceBundle at the given location.  All
     * subsequent bundles will be pushed down one space.
