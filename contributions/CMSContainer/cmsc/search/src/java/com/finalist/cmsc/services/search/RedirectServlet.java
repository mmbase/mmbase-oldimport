...
public class RedirectServlet extends BridgeServlet {
   ...
   private void doRedirect(HttpServletRequest request, HttpServletResponse response) throws IOException {
      ...
      /*************************************************************************
       * All types above this comment should all be redirected and not forwarded
       * with a dispatcher *****************************
       */
      if (redirect != null) {
      ...
      if (ContentElementUtil.isContentElement(node)) {
         ...
         if (pageInfo != null) {
            ...
            // contentportlet
            if (!("contentelement".equals(pageInfo.getParametername()) && elementId
                  .equals(pageInfo.getParametervalue()))) {
      ...
      if (redirect != null) {
         if (this.forwardRequest) {
            if (!redirect.startsWith("/") && redirect.indexOf("://") > -1 && ServerUtil.useServerName()) {
               // not a valid forward dispatch url, but it might be converted to
               // one.
            ...
            try {
               ...
               PortalEnvironment insternalEnv = new PortalEnvironment(internalRequest, response, this
                     .getServletConfig());
