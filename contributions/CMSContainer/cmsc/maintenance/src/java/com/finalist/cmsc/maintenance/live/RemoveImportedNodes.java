...
/**
 * Removes suspected imported nodes which are not in the remotenodes table anymore 
 */
public class RemoveImportedNodes extends SqlAction {
    ...
    public String getSql() {
        return "SELECT " + getTableField(table,"*") +
                " FROM " + getTable(table) + 
           " LEFT JOIN " + getTable("remotenodes") +
                  " ON " + getTableField(table,"number") + " = " + getTableField("remotenodes","destinationnumber") +
               " WHERE " + getTableField("remotenodes","destinationnumber") + " IS NULL ";
    }
    ...
    public String process(ResultSet rs) throws BridgeException, SQLException {
        ...
        if (ServerUtil.isLive()) {
            ...
            //Cloud remoteCloud = CloudManager.getCloud(localCloud, stagingServer);
            ...
            while (rs.next()) {
               ...
               for (Iterator iter = relations.iterator(); iter.hasNext();) {
                  ...
                  if (PublishManager.isImported(localCloudInfo, rel)) {
                      ...
                      result.append("Unlink relation " + rel.getNumber() 
                              + " with source " +  rel.getIntValue("snumber") + " and destination " + rel.getIntValue("dnumber") + " <br />");
                  }
                  if ("remove".equals(action)) {
                      rel.delete(false);
                  }
