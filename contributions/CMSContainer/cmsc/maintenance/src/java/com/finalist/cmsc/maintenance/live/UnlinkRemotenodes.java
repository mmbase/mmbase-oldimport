...
public class UnlinkRemotenodes extends SqlAction {
   ...
   public String getSql() {
     ...
     return "SELECT " + getTableField("remotenodes","*") +
             " FROM " + getTable("remotenodes") + 
        " LEFT JOIN " + getTable("object") +
               " ON " + getTableField("remotenodes","destinationnumber") + " = " + getTableField("object","number") +
            " WHERE " + getTableField("object","number") + " IS NULL " +
               "and " + getTableField("remotenodes","destinationcloud") + " = " + liveNumber;
   }
   ...
   public String process(ResultSet rs) throws BridgeException, SQLException {
      ...
      if (ServerUtil.isLive()) {
          ...
          //Cloud remoteCloud = CloudManager.getCloud(getCloud(), stagingServer);
          ...
          while (rs.next()) {
             ...
             result.append("Remote node " + remoteNodeNumber 
                   + " with staging " +  stagingNodeNumber + " and live " + liveNodeNumber + " <br />");
