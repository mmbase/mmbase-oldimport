...
//          CloudManager.getCloudNumber(sourceCloudInfo, sourceCloudInfo);
   ...
   public void stepOne() {
      ...
      for (int x = 0; x < sourceNodeManagerList.size(); x++) {
         sourceNodeManagers.add(sourceNodeManagerList.getNodeManager(x)
                                                            .getName());
      }

      addMessage("1: node manager count in source cloud:" +
         sourceNodeManagers.size());

      List<String>  destinationNodeManagers = new ArrayList<String>();

      for (int x = 0; x < destinationNodeManagerList.size(); x++) {
         destinationNodeManagers.add(destinationNodeManagerList.getNodeManager(
               x).getName());
      }

      addMessage("1: node manager count in destination cloud:" +
         destinationNodeManagers.size());
      ...
      //keep only the node from the source cloud that are also in the destination cloud
   ...
   public void stepTwo() {
      ...
      while (iter.hasNext()) {
         ...
         if (skipBuilders.indexOf(nodeManagerName) == -1) {
            ...
            NodeList sourceNodes = sourceCloudInfo.getCloud().getNodeManager(nodeManagerName)
   ...
   public static void main(String[] argv) {
      if (argv.length != 2) {
         System.err.println(
            "Usage CloudDuplicator rmi://127.0.0.1:1111/staging rmi://127.0.0.1:1111/live");
         System.exit(1);
      }
