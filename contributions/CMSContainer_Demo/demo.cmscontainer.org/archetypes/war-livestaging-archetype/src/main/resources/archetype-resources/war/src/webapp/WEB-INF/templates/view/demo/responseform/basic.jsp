...
<mm:cloud method="asis">
	...
	<c:choose>
		<c:when test="${dollar}{empty isConfirmPage}">
			<c:set var="emailerrormessage">
				<c:out value="${dollar}{errormessages['sendemail']}"/>
			</c:set>
			<c:if test="${dollar}{!empty emailerrormessage}">
					...
					<mm:relatednodescontainer type="formfield" role="posrel" >							
						...
						<mm:relatednodes>	
				            ...
				            <c:set var="fieldvalue">
		                       		<c:out value="${dollar}{requestScope[fieldidentifier]}"/>
						    </c:set>                                
							
				            <c:if test="${dollar}{empty fieldvalue && fieldtype!=4 && fieldtype!=5}">				           
		 					...
		 					<tr> 					
								<td>								
									...
									<c:choose>	
										...
										<c:when test="${dollar}{fieldtype==4}">
											<mm:node number="${fieldnumber}" notfound="skip">							                	
												...
												<mm:relatednodes> 
													...
													<c:if test="${dollar}{((usedefault==1) && (empty fieldvalue)) || (fieldvalue eq standardvalue)}">
										...
										<c:when test="${dollar}{fieldtype==5}">
											<mm:node number="${fieldnumber}" notfound="skip">
									...
									<c:if test="${dollar}{mandatory == 1}">
										<font size="1" color="red"><b>*</b></font>
									</c:if>										
									<c:set var="errormessagekey" >
										<c:out value="${dollar}{errormessages[fieldidentifier]}"/>
									</c:set>
									<c:if test="${dollar}{!empty errormessagekey}">
