...
<cmsc:list-pages var="pages" origin="${dollar}{listPath[1]}" />

<div id="navleft">
   <c:forEach var="page" items="${pages}">
      <c:choose>
         <c:when test="${dollar}{listPath[2].id eq page.id and fn:length(listPath) eq 3}">        
            <div class="selected">
      ...
      <c:if test="${dollar}{fn:length(listPath) > 2 and (listPath[2] eq page or listPath[3] eq page)}">
         <cmsc:list-pages var="subchannels" origin="${page}"/>
         <c:if test="${dollar}{not empty subchannels}">
            <c:forEach var="subchan" items="${subchannels}">
               <c:choose>
                  <c:when test="${dollar}{listPath[3].id eq subchan.id}">             
                     <div class="selected2">
