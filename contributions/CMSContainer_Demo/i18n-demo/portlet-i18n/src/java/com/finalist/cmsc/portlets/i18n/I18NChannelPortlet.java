...
public class I18NChannelPortlet extends ContentChannelPortlet {
    ...
    protected int countContentElements(RenderRequest req, List<String> contenttypes, String channel, int offset,
            String orderby, String direction, String archive, int maxNumber, int year,
            int month, int day, boolean useLifecycle) {

        Node chan = getCloud().getNode(channel);
        
        NodeQuery query = RepositoryUtil.createLinkedContentQuery(chan, contenttypes, orderby,
                direction, useLifecycle, archive, offset, maxNumber, year, month, day);
    ...
    protected List<ContentElement> getContentElements(RenderRequest req, List<String> contenttypes, String channel,
            int offset, String orderby, String direction, String archive, int maxNumber,
            int year, int month, int day, boolean useLifecycle) {
    ...
    private void addLanguageConstraint(RenderRequest req, NodeQuery query) {
        ...
        if (!locales.isEmpty()) {
            ...
            FieldValueConstraint constraint = SearchUtil.createEqualConstraint(query, keyField, locales.get(0).getLanguage());
