...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  version="1.0">
  ...
  <xsl:template name="extrastyle">
   ...
   <xsl:template name="extrajavascript">
     <script language="javascript" src="{$referrerdir}style/streammanager.js.jsp?dir={$referrerdir}&amp;fragment={/wizard/form/list/item[@role='parent']/@destination}"><xsl:comment>help IE</xsl:comment></script>
  </xsl:template>
