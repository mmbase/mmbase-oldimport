...
public class AudioTmp extends Vwm implements MMBaseObserver,VwmServiceInterface {

	public AudioTmp() {
		System.out.println("AudioTmp ready for action");
	}


	public boolean probeCall() {
		System.out.println("AudioTmp probe");
		return(true);
	}

	public boolean nodeRemoteChanged(String number,String builder,String ctype) {
		System.out.println("AudioTmp -> remote node changed "+number);
		return(nodeChanged(number,builder,ctype));
	}

	public boolean nodeLocalChanged(String number,String builder,String ctype) {
		System.out.println("AudioTmp -> local node changed "+number);
		return(nodeChanged(number,builder,ctype));
	}

	public boolean nodeChanged(String number,String builder, String ctype) {
		System.out.println("AudioTmp -> sees that : "+number+" has changed type="+ctype);
		return(true);
	}

	public boolean fileChange(String service,String subservice,String filename) {
		System.out.println("AudioTmp frontend change -> "+filename);
	...
	public boolean fileChange(String number, String ctype) {
		// System.out.println("AudioTmp -> fileChange="+number+" "+ctype);
	...
	protected boolean handleTmp(MMObjectNode filenode,int status,String ctype) {
		System.out.println("AudioTmp -> handleTmp "+filenode+" : "+status+" "+ctype);
		if (ctype.equals("d")) return(false);
		switch(status) {
			case 1:  // Verzoek
				System.out.println("AudioTmp-> Tmp verzoek");
...
//				filenode.commit();
				...
				// end stuff
				System.out.println("AudioTmp-> doing Tmp stuff");
				filenode.setValue("status",3);
				filenode.commit();
				break;
			case 2:  // Onderweg
				System.out.println("AudioTmp-> Tmp Onderweg");
				break;
			case 3:  // Gedaan
				System.out.println("AudioTmp-> Tmp Done");
	...
	protected boolean handleUpload(MMObjectNode filenode,int status,String ctype) {
		...
		switch(status) {
			case 1:  // Verzoek
				System.out.println("AudioTmp-> main verzoek");
				doUploadRequest(filenode);
				break;
			case 2:  // Onderweg
				System.out.println("AudioTmp-> main Onderweg");
				break;
			case 3:  // Gedaan
				System.out.println("AudioTmp-> main Done");
				// Eleminate TMP files
				cleanupTmp(filenode);
				break;
		}
	...
	protected boolean generateUploadNode(MMObjectNode filenode) {
		...
		mainnode=bul.getNewNode("AudioTmp");
		...
		System.out.println("AudioTmp -> inserting "+mainnode);
		bul.insert("AudioTmp",mainnode);
		return(true);
	}
	...
	protected boolean doUploadRequest(MMObjectNode filenode) {
		...
		if (e.hasMoreElements()) {
			...
			while (e.hasMoreElements()) {
				...
				System.out.println("AudioTmp -> tmp "+tnode);
				// DO COPY
				System.out.println("AudioTmp -> copying ... ");
				cp.copy(tnode,filenode);
				System.out.println("AudioTmp -> end copying ... ");
			}
	...
	protected boolean cleanupTmp(MMObjectNode filenode) {
		...
		while (e.hasMoreElements()) {
			tnode=(MMObjectNode)e.nextElement();
			System.out.println("AudioTmp -> for us "+tnode);
			// Remove file ?
			CopyServices cp=(CopyServices)Vwms.mmb.getMMObject("cpservices");
//			cp.remove(tnode);
